// Package database provides PostgreSQL database connection management using pgx.
package database
...
// multiTracer chains multiple pgx tracers together.
...
func New(cfg *config.Config, logger *zerolog.Logger, loggerService *loggerConfig.LoggerService) (*Database, error) {
	...
	if cfg.Primary.Env == "local" {
		globalevel := logger.GetLevel()
		pgxLogger := loggerConfig.NewPgxLogger(globalevel)
		if pgxPoolConfig.ConnConfig.Tracer != nil {
			localTracer := &tracelog.TraceLog{
				Logger:   pgxzero.NewLogger(pgxLogger),
				LogLevel: tracelog.LogLevel(loggerConfig.GetPgxTraceLogLevel(globalevel)),
			}
